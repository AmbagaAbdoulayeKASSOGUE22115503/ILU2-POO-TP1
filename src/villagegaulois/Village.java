...
public class Village {
	...
	public String installerVendeur(Gaulois vendeur, String produit, int nbProduit) {
		...
		messRetour.append(vendeur.getNom() + " cherche un endroit pour vendre " + nbProduit + " " + produit + ".\n");

		int indiceEtalLibre = marche.trouverEtalLibre(); // pour éviter le numéro d'étal 0.
		messRetour.append("Le vendeur " + vendeur.getNom() + " vend des " + produit + " à l'étal n°"
				+ (indiceEtalLibre + 1) + ".\n");
	...
	public String rechercherVendeursProduit(String produit) {
		...
		// traitement du cas où il n'y a pas de vendeur qui propose ce produit ou il y'a
		// un seul vendeur qui propose ce produit.
		...
		if (etalsVenteProduit.length == 1)
			return (resultat.append("Seul le vendeur " + etalsVenteProduit[0].getVendeur().getNom() + " propose des "
					+ produit + " au marché.\n")).toString();
	...
	private static class Marche {
		...
		private Etal[] trouverEtals(String produit) {
			...
			for (int i = 0, ind = 0; i < etals.length; i++) {
				if (etals[i].isEtalOccupe() && etals[i].contientProduit(produit))
		...
		private String afficherMarche() {
			...
			for (int i = 0; i < etals.length; i++) {
				etal = etals[i].afficherEtal();
				if (etal.equals("L'étal est libre")) {
					nbEtalVide++;
					continue;
				}
				affichage += etal + "\n";
